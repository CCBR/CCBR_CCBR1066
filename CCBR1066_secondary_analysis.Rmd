...
#####################################
# KMeans analysis for group A only
#####################################
```{r}
deg_list = c(paste0(deg_dir, "DEG_Mock-KO_0.5_0.5/limma_DEG_Mock-KO_all_genes.txt"),
                                paste0(deg_dir, "DEG_WT-KO_0.5_0.5/limma_DEG_WT-KO_all_genes.txt"),
...
remove(merged_df)
for (deg in deg_list){
  tmp_df = read.csv(deg,sep="\t")[,c("ensid_gene","pvalue","gsea_ranking_score")]
  tmp_df = tmp_df %>%
    separate(ensid_gene, c("eid", "gene"), "\\|")
  
  tmp_df$eid2 = tmp_df$eid
  tmp_df = tmp_df %>%
    separate(eid2, c("eid2", "sub"), "\\.")
...
#input file from output of http://bioinformatics.sdstate.edu/idep/
km_df = read.csv("~/../../Volumes/projects/ccbr1066/analysis/kmeans/Kmeans_bygroup.csv")
km_df = subset(km_df,Cluster=="A")
km_df = km_df[,c("Eid","Symbol","MockKOfc","WTKOfc","Y318AKOfc")]
km_df[is.na(km_df)] <- ""

#add variant info, pvalue to keg output
for(rowid in rownames(km_df)){
  search_eid = subset(merged_df,eid==km_df[rowid,"Eid"])$gene[1]
  search_eid2 = subset(merged_df,eid2==km_df[rowid,"Eid"])$gene[1]
  
  if(is.na(search_eid)){
    km_df[rowid,"Variant"] = subset(merged_df,eid2==km_df[rowid,"Eid"])$gene[1]
    
    mock_p = subset(merged_df,eid2==km_df[rowid,"Eid"] & contrast=="Mock.KO")$pvalue
    wt_p = subset(merged_df,eid2==km_df[rowid,"Eid"] & contrast=="WT.KO")$pvalue
    y_p = subset(merged_df,eid2==km_df[rowid,"Eid"] & contrast=="Y318A.KO")$pvalue
    
    if(length(mock_p)!=0){
      km_df[rowid,"Mock.KO.p"] = mock_p
    }
    
    if(length(wt_p)!=0){
      km_df[rowid,"WT.KO.p"] = wt_p
    }
    
    if(length(y_p)!=0){
      km_df[rowid,"Y318A.KO.p"] = y_p
    }
     
  } else{
    km_df[rowid,"Variant"] = subset(merged_df,eid==km_df[rowid,"Eid"])$gene[1]
...
km_df_out = km_df[,c("Eid","Gene_Transcript","Variant",
                           "Mock.KO.fc","Mock.KO.p",
                           "WT.KO.fc","WT.KO.p",
                           "Y318A.KO.fc","Y318A.KO.p")]
...
write.csv(km_df_out,paste0(out_dir,"kmeans_stats_all.csv"))
write.csv(km_df_sig,paste0(out_dir,"kmeans_stats_sig.csv"))
```
